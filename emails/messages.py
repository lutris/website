...
def send_game_accepted(user, game):
    ...
    context = {"username": user.username, "name": game.name, "game_url": game.get_absolute_url()}
...
def notify_rejected_installer(installer, review, user):
    ...
    context = {"username": user.username, "installer": installer, "review": review}
    subject = f"Your installer for {installer} wasn't published"
    send_email("installer_rejected", context, subject, user.email)
