...
def script_is_not_the_default_one(installer):
    ...
    if script == DEFAULT_INSTALLER:
        return (False, "Do not submit the default installer.")
    return SUCCESS
    ...
    if "installer" in script:
        ...
        for step in script["installer"]:
            if isinstance(step, dict) and len(step.keys()) > 1:
                return (
                    False,
                    "Installer step %s shouldn't have more "
...
def scummvm_has_gameid(installer):
    ...
    if "game" not in script:
        return (False, "Missing section 'game'")
    if "game_id" not in script["game"]:
        return (False, "ScummVM game should have a game identifier in the 'game' section")
    return SUCCESS
...
def winesteam_scripts_use_correct_prefix(installer):
    ...
    if "$USER" in script["game"].get("prefix", ""):
        return (False, "Do not create the prefix in the home folder, use $GAMEDIR/prefix")
    if script["game"].get("prefix", "").strip() == "$GAMEDIR":
        return (False, "Do not create the prefix directly in the game folder, use $GAMEDIR/prefix")
    return SUCCESS
    ...
    for file_info in script.get("files") or []:
        ...
        if isinstance(file_meta, dict):
            if "url" not in file_meta or "filename" not in file_meta:
                return (False, "Files should have a url and filename parameters.")
    return SUCCESS
...
def tasks_have_names(installer):
    ...
    for step in script.get("installer") or []:
        ...
        if step_name == "task":
            if not arguments:
                return (False, "Empty task %s" % step_name)
            if "name" not in arguments:
                return (False, "All tasks should have a name.")
    return SUCCESS
...
def no_home_in_files(installer):
    ...
    for file_info in script.get("files") or []:
        ...
        if url and str(url).startswith("/home"):
            return (False, "Don't reference files from your own home folder.")
    return SUCCESS
...
def all_files_are_used(installer):
    ...
    for file_info in script["files"]:
        if not isinstance(file_info, dict):
            return (False, "Files should be a mapping: 'file_id: url'")
        file_id = next(iter(file_info.keys()))
        if file_id not in installer_text:
            return (False, f"File {file_id} is referenced but not used anywhere")
    return SUCCESS
...
def uses_a_valid_wine_version(installer):
    ...
    if not script_version:
        return SUCCESS
    versions = [
        f"{v.version}-{v.architecture}" for v in RunnerVersion.objects.filter(runner__slug="wine")
    ]
    if script_version not in versions:
        return (False, f"Wine version {script_version} is not available from Lutris")
    return SUCCESS
