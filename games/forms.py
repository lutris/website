...
class InstallerForm(forms.ModelForm):
    ...
    def clean_content(self):
        ...
        if yaml_data and "script" in yaml_data:
            yaml_data = yaml_data["script"]
        if not yaml_data:
            raise forms.ValidationError("The installer content cannot be empty")
    ...
    def clean_version(self):
        ...
        existing = models.Installer.objects.filter(game=self.instance.game, version=version)
        if self.instance.base_installer:
            existing = existing.exclude(id=self.instance.base_installer.id)
        if existing.exists():
