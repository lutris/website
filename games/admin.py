...
class GameFilter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        if self.value() == "only-games-with-changes":
            subqueryset = (
                queryset.values("change_for__id").filter(change_for__isnull=False).distinct()
            )
...
class InstallerHistoryAdmin(admin.StackedInline):
    ...
    extra = 0
    readonly_fields = ("installer", "user", "runner", "version", "description", "notes", "content")
...
class InstallerAdmin(admin.ModelAdmin):
    ...
    inlines = [InstallerHistoryAdmin]

    def game_link(self, obj):
        return mark_safe(
            "<a href='{0}'>{1}<a/>".format(
                reverse("admin:games_game_change", args=(obj.game.id,)), obj.game
            )
        )
...
class IssueReplyInline(admin.StackedInline):
    ...
    extra = 1
    readonly_fields = ("submitted_by", "submitted_on", "issue")
...
class InstallerIssueAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ("submitted_by", "submitted_on", "installer", "game_link")

    inlines = [IssueReplyInline]
...
class GameAdmin(admin.ModelAdmin):
    ...
    inlines = [GameLinkAdmin, GameAliasAdmin, ShaderCacheAdmin]
...
class GameSubmissionAdmin(admin.ModelAdmin):
    ...
    def user_link(self, obj):
        return mark_safe(
            "<a href='{0}'>{1}</a>".format(
                reverse("admin:accounts_user_change", args=(obj.user.id,)), obj.user
            )
        )
...
class GameMergeSuggestionAdmin(admin.ModelAdmin):
    ...
    def other_game_link(self, obj):
        return mark_safe(
            "<a href='{0}'>{1}</a>".format(
                reverse("admin:games_game_change", args=(obj.other_game.id,)), obj.other_game
            )
        )
